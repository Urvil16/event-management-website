<?php
    ...
    if(isset($_REQUEST['submit']))
    {
        ...
        {
            $idRes = mysqli_query($con, "SELECT IFNULL(MAX(id),0)+1 AS next FROM birthday");
            if($idRes !== false)
            {
                $row = mysqli_fetch_assoc($idRes);
                $nextId = (int)$row['next'];
                $stmt = mysqli_prepare($con, "INSERT INTO birthday(id, img, nm, price) VALUES(?, ?, ?, ?)");
                if($stmt !== false)
                {
                    ...
                    if($ok)
                    {
                        ...
                        echo '<script type="text/javascript">window.location="hecathon_disp.php";</script>';
                    }
                    else
                    {
...
<div class="codes">
<div class="container"> 
<h3 class='w3ls-hdg' align="center">ADD HACKATHON</h3>
<div class="grid_3 grid_4">
				<div class="tab-content">
					...
					<input type="submit" name="submit" class="btn" value="SEND"> <input type="button" value="DISPLAY" class="btn my" onClick="javascript:location.href='hecathon_disp.php'"/>
					</div>
					</form>
					</div>
				</div>
