...
if(isset($_POST['submit']))
{
	...
	$name = trim($_POST['nm']);
	$pwd = $_POST['pwd'];
	$stmt = mysqli_prepare($con, "SELECT nm, pswd FROM admin WHERE nm = ?");
	mysqli_stmt_bind_param($stmt, "s", $name);
	mysqli_stmt_execute($stmt);
	mysqli_stmt_store_result($stmt);
	if (mysqli_stmt_num_rows($stmt) > 0) {
		mysqli_stmt_bind_result($stmt, $db_nm, $db_pswd);
		mysqli_stmt_fetch($stmt);
		if (password_verify($pwd, $db_pswd) || $pwd === $db_pswd) {
			$_SESSION['admin'] = $db_nm;
...
<body>
		...
		<nav class="navbar navbar-default">
			<div class="container">
				<div class="navbar-header navbar-left">
					<h1>
					<img src="../images/logo.png?v=<?php echo time(); ?>"></h1>
				</div>
				<!-- navigation--> 
	...
	<div class="footer">
		<div class="container">
			<h3 class="w3ltitle"><span>Admin Login</span></h3>
	...
	<div class="footer">
		...
		<div class="copy-right w3-agile-text">
			<div class="container">
				...
				<p>© 2025 Eventally, Ganpat University , All rights reserved | </p>	
			</div>
		</div>
		<!-- //copy-right -->
	</div>
