<?php
	include_once("header.php");
	include('../Database/connect.php');
	include('session.php');
	if(isset($_REQUEST['submit']))
	{
        $id=(int)$_REQUEST['id'];
        $fnm=$_FILES["image"]["name"];
        $nm=$_REQUEST['nm'];
        $pr=(int)$_REQUEST['price'];
        
        move_uploaded_file($_FILES["image"]["tmp_name"],"../images/" .$_FILES["image"]["name"]);
        $stmt = mysqli_prepare($con, "UPDATE anniversary SET img=?, nm=?, price=? WHERE id=?");
        mysqli_stmt_bind_param($stmt, "ssii", $fnm, $nm, $pr, $id);
        $ok = mysqli_stmt_execute($stmt);
        mysqli_stmt_close($stmt);
        if($ok)
		{
			echo "<script> alert('Updated');</script>";
			echo '<script type="text/javascript">window.location="Google_disp.php";</script>';
		}
		else
		{
			echo "<script> alert('Not Updated');</script>";
		}
	}
	$id=(int)$_REQUEST['id'];
	$se=mysqli_query($con,"select * from anniversary where id=$id");
	if($se === false)
	{
		die("Error: " . mysqli_error($con));
	}
	$row=mysqli_fetch_array($se);
	if($row === false || $row === null)
	{
		die("Error: Record not found");
	}
	
?>
<link href="../css/style.css" rel="stylesheet" type="text/css" />

<div class="codes">
<div class="container"> 
<h3 class='w3ls-hdg' align="center">EDIT Google</h3>
<div class="grid_3 grid_4">
				<div class="tab-content">
					<div class="tab-pane active" id="horizontal-form">
						<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
							<input type="hidden" name="id" value="<?php echo $id; ?>">
							<div class="form-group">
								<label for="focusedinput" class="col-sm-2 control-label">Enter Image :</label>
								<div class="col-sm-8">
								<input type="file"  name="image">
								</div><div align="center">
								<img src="../images/<?php echo $row['img']; ?>" height="200"  width="200"/></div>		
							</div>
							<div class="form-group">
								<label for="focusedinput" class="col-sm-2 control-label">Enter Price :</label>
								<div class="col-sm-8">
									<input type="text" class="form-control1" value="<?php echo $row['price']; ?>" name="price" id="focusedinput" placeholder="Theme Price" >
								</div>
							</div>
							<div class="form-group">
								<label for="txtarea1" class="col-sm-2 control-label">Enter Name :</label>
								<div class="col-sm-8"><input type="text" value="<?php echo $row['nm']; ?>" name="nm" id="focusedinput" class="form-control1">
								</div>
							</div>
					<div class="contact-w3form" align="center">
					<input type="submit" name="submit" class="btn" value="UPDATE"> <input type="button" value="DISPLAY" class="btn my" onClick="javascript:location.href='Google_disp.php'" />
					</div>
					</form>
					</div>
				</div>
			</div>
		</div>
	</div>
	</div>
	<?php
		include_once("footer.php");
	?>